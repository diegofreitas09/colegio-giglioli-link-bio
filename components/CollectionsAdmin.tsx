...
export default function CollectionsAdmin() {
  ...
  async function upload(file: File, section: Section) {
    ...
    const { error } = await supabase.storage.from("site-media-public").upload(objectPath, file, { cacheControl: "3600", upsert: false });
  ...
  async function createItem(e: FormEvent<HTMLFormElement>) {
    ...
    const section = String(fd.get("section")) as Section;
    const titulo = String(fd.get("titulo") || "").trim();
    if (!titulo) return setNotice("Informe o título.");

    setNotice("Salvando conteúdo...");
    try {
      const file = fd.get("imagem");
      const imagem_url = file instanceof File && file.size ? await upload(file, section) : null;
      const { error } = await supabase.from("site_collections").insert({
        ...
        ordem: Number(fd.get("ordem") || 100),
        inicio_em: String(fd.get("inicio_em") || "").trim() ? new Date(String(fd.get("inicio_em"))).toISOString() : null,
        fim_em: String(fd.get("fim_em") || "").trim() ? new Date(String(fd.get("fim_em"))).toISOString() : null
      });
      if (error) throw error;
      form.reset();
  ...
  async function editItem(item: Item) {
    ...
    const descricao = prompt("Descrição / legenda", item.descricao || "");
    if (descricao === null) return;
    const link_url = prompt("Link de acesso (opcional)", item.link_url || "");
    if (link_url === null) return;
    const link_label = prompt("Texto do botão (opcional)", item.link_label || "");
    if (link_label === null) return;
  ...
  if (!sessionChecked) return <main className="grid min-h-screen place-items-center bg-[#061329] text-white">Carregando...</main>;
  if (!authorized) {
    return (
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mt-7 grid gap-6 lg:grid-cols-[.9fr_1.1fr]">
          <form onSubmit={createItem} className="h-fit rounded-[30px] border border-white/10 bg-white/6 p-5 backdrop-blur-xl sm:p-7">
            <h2 className="text-xl font-black">Novo conteúdo</h2>
            <div className="mt-5 grid gap-4">
              <label className="grid gap-2 text-xs font-black text-slate-300">Módulo
                <select name="section" required className="rounded-2xl border border-white/10 bg-[#0b2448] px-4 py-3 text-white">
              ...
              <label className="grid gap-2 text-xs font-black text-slate-300">Título
                <input name="titulo" required className="rounded-2xl border border-white/10 bg-[#0b2448] px-4 py-3 text-white" />
              </label>
              <label className="grid gap-2 text-xs font-black text-slate-300">Descrição / legenda
                <textarea name="descricao" rows={4} className="rounded-2xl border border-white/10 bg-[#0b2448] px-4 py-3 text-white" />
              </label>
              <label className="grid gap-2 text-xs font-black text-slate-300">Foto
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <label className="grid gap-2 text-xs font-black text-slate-300">Link de acesso
                ...
                <label className="grid gap-2 text-xs font-black text-slate-300">Texto do botão
                  <input name="link_label" placeholder="Acessar boletim" className="rounded-2xl border border-white/10 bg-[#0b2448] px-4 py-3 text-white" />
                </label>
              </div>
              <div className="grid gap-4 sm:grid-cols-2">
              ...
              <label className="grid gap-2 text-xs font-black text-slate-300">Ordem
              ...
              {notice ? <p className="rounded-2xl bg-white/7 px-4 py-3 text-xs font-bold text-sky-200">{notice}</p> : null}
            </div>
          </form>
          ...
          <section>
            ...
            <div className="mt-4 grid gap-4">
              ...
              {visible.map((item) => (
                <article key={item.id} className="grid gap-4 rounded-[26px] border border-white/10 bg-white/6 p-4 sm:grid-cols-[150px_1fr]">
                  <div className="aspect-[4/3] overflow-hidden rounded-2xl bg-[#0b2448]">
                    {item.imagem_url ? <img src={item.imagem_url} alt="" className="h-full w-full object-cover" /> : <div className="grid h-full place-items-center text-4xl">🖼️</div>}
                  </div>
                  <div className="min-w-0">
                    ...
                    {item.section === "campanhas" && (item.inicio_em || item.fim_em) ? <p className="mt-2 text-xs font-bold text-slate-400">{item.inicio_em ? `Início: ${localDate(item.inicio_em)}` : ""} {item.fim_em ? `• Fim: ${localDate(item.fim_em)}` : ""}</p> : null}

                    <div className="mt-4 flex flex-wrap gap-2">
                      <button onClick={() => editItem(item)} className="rounded-full bg-white px-3 py-2 text-[11px] font-black text-[#0a2856]">Editar</button>
              ...
              {!loading && !visible.length ? <div className="rounded-[26px] border border-dashed border-white/15 p-8 text-center text-sm font-bold text-slate-400">Nenhum conteúdo cadastrado neste módulo.</div> : null}
            </div>
          </section>
        </div>
      </div>
